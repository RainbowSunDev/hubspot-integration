...
import { type HubspotUser, getUsers } from './users';
import { HubspotError } from './commons/error';

const validToken = 'token-1234';
const maxPage = 3;

const users: HubspotUser[] = Array.from({ length: 5 }, (_, i) => ({
  id: `id-${i}`,
  username: `username-${i}`,
...
describe('auth connector', () => {
  describe('getUsers', () => {
    ...
    beforeEach(() => {
      server.use(
        http.get('https://Hubspot.com/api/v1/users', ({ request }) => {
    ...
    test('should throws when the token is invalid', async () => {
      await expect(getUsers('foo-bar', 0)).rejects.toBeInstanceOf(HubspotError);
    });
  });
});
